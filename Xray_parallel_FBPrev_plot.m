...
for jjj = 1:Nang
    ...
    %tmpsino(:,1:(end-jjj+1)) = sinogram(:,1:(end-jjj+1));
    tmpsino(:,jjj:end) = sinogram(:,jjj:end);
    FBP = iradon(tmpsino,rotangs_d-90);
    FBP = FBP(2:(end-1),2:(end-1));
    FBP = max(0,FBP);
    FBP = FBP/max(FBP(:));
    FBP_plot = [zeros(Nph);FBP;zeros(Nph)];
    FBP_plot = [zeros(3*Nph,Nph),FBP_plot,zeros(3*Nph,Nph)];
    imagesc(FBP_plot.^gammacorr,[0,1])
    colormap gray
    hold on
