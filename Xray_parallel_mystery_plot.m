% Plot a basic illustration of continuous Radon transform using an
% emoji digital phantom. Here we do not show the object under imaging;
% the purpose is to create a video that shows the parallel beam tomographic
% data but leaves the object to be guessed by the viewer.
%
...
for jjj = 1:Nang
    ...
    % Plot "cover" that hides the object under imaging
    p1 = patch(1.5*Nph+.42*Nph*cos(linspace(0,2*pi,50)),1.5*Nph+.42*Nph*sin(linspace(0,2*pi,50)),[.8 .8 .8],'LineWidth',2,'EdgeColor',[.5 .5 .5]);
    t1 = text(1.29*Nph,1.53*Nph,'?','fontsize',65,'fontname','helvetica');
    set(t1,'color',[.5 .5 .5])
